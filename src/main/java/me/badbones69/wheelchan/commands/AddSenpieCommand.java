...
public class AddSenpieCommand {
    ...
    public static void runCommand(MessageReceivedEvent e) {
        ...
        EmbedBuilder embed = Messages.ADD_SENPIE_DESCRIPTION.getMessage(guild, "%senpie%", user.getAsMention());
        if (wheelChan.isSensei(user, e.getGuild())) {
            if (!message.getMentionedUsers().isEmpty()) {
                ...
                if (!newSenpais.isEmpty()) {
                    ...
                    embed = Messages.ADD_SENPIE.getMessage(guild, "%senpais%", getSenpieList(newSenpais));
                } else {
                    embed = Messages.ALREADY_MY_SENPIE.getMessage(guild, "%senpais%", getSenpieList(message.getMentionedUsers()));
                }
            }
        } else {
